...
if [[ $HEAD_NODE -eq 1 ]]; then
    ...
    # run EP/D workers
    python3 "$REPO_ROOT/src/components/multimodal/components/worker.py" --model $MODEL_NAME --worker-type encode_prefill --enable-disagg --tensor-parallel-size=8 --max-model-len=208960 &
else
    # run decode worker on non-head node
    python3 "$REPO_ROOT/src/components/multimodal/components/worker.py" --model $MODEL_NAME --worker-type decode --enable-disagg --tensor-parallel-size=8 --max-model-len=208960 &
fi
