...
mod tests {
    ...
    fn make_test_config(
        ...
        host_cache_gb: f64,
        disk_num_blocks: usize,
        disk_cache_gb: f64,
    ) -> KvbmLeaderConfig {
        KvbmLeaderConfig::builder()
            .barrier_id_prefix("test".to_string())
            .world_size(1)
            .leader_init_timeout_secs(1)
            .drt(None)
            .host_blocks_config(KvbmLeaderNumBlocksConfig {
                num_blocks_overriden: host_num_blocks,
                cache_size_in_gb: host_cache_gb,
            })
            .disk_blocks_config(KvbmLeaderNumBlocksConfig {
                num_blocks_overriden: disk_num_blocks,
                cache_size_in_gb: disk_cache_gb,
            })
            .build()
            .unwrap()
    }
    ...
    #[test]
    fn test_sanity_check_errors_when_no_cpu_and_no_disk() {
        let config = make_test_config(0, 0.0, 0, 0.0);
        let result = config.sanity_check();
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
        assert!(error_msg.contains("No CPU memory configured"));
        assert!(error_msg.contains("DYN_KVBM_CPU_CACHE_GB"));
        assert!(error_msg.contains("DYN_KVBM_CPU_CACHE_OVERRIDE_NUM_BLOCKS"));
    }

    #[test]
    fn test_sanity_check_errors_when_no_cpu_but_disk_set() {
        let config = make_test_config(0, 0.0, 10, 1.0);
        let result = config.sanity_check();
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
        assert!(error_msg.contains("CPU memory must be configured before disk memory"));
        assert!(error_msg.contains("CPU blocks: 0 (cache: 0 GB)"));
        assert!(error_msg.contains("Disk blocks: 10 (cache: 1 GB)"));
        assert!(error_msg.contains("DYN_KVBM_CPU_CACHE_GB"));
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_blocks_set() {
        let config = make_test_config(10, 0.0, 0, 0.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_cache_set() {
        let config = make_test_config(0, 1.0, 0, 0.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_and_disk_set() {
        let config = make_test_config(10, 2.0, 5, 1.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_blocks_and_disk_cache_set() {
        let config = make_test_config(5, 0.0, 0, 2.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_cache_and_disk_blocks_set() {
        let config = make_test_config(0, 1.5, 3, 0.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_cache_and_disk_cache_set() {
        let config = make_test_config(0, 1.0, 0, 2.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_blocks_and_disk_blocks_set() {
        let config = make_test_config(5, 0.0, 3, 0.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_all_set() {
        let config = make_test_config(5, 1.0, 3, 2.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_blocks_nonzero_cache_zero() {
        let config = make_test_config(1, 0.0, 0, 0.0);
        assert!(config.sanity_check().is_ok());
    }

    #[test]
    fn test_sanity_check_ok_when_cpu_cache_nonzero_blocks_zero() {
        let config = make_test_config(0, 0.1, 0, 0.0);
        assert!(config.sanity_check().is_ok());
    }
}
